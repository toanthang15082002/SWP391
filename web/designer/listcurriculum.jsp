                    ...
                    <div class="content__detail col-lg-10" >
                        <div class="content__detail-addSyllabus">
                            ...
                            <div class="title text-center">
                                <h1>List Curriculum</h1>
                            </div>
                            <form action="searchcurriculum" method="post">
                                <div class="d-flex justify-content-center" >
                                    <div class="form-outline col-lg-8">
                                        <input type="search" class="form-control" name="curText" placeholder="Enter curriculum code or curriculum name" style="height: 100%"/>
                                    </div>
                                    <button type="submit" class="btn btn-primary col-lg-2" >
                                        <i class="fas fa-search"></i>
                                    </button>
                                    <button style="margin-left: 5px" class="btn btn-primary" type="button">
                                        <a style="color: white" href="addcurriculum">Add Curriculum</a>
                                    </button>
                                </div>
                            <c:if test="${requestScope.listCurriculumPaging.size() == 0}">
                        ...
                        <c:if test="${requestScope.listCurriculumPaging.size() > 0}">
                            <table class="table table-striped" style="font-size: 1.2em">
                                ...
                                <tbody>
                                    <c:forEach items="${listCurriculumPaging}" var="o">
                                        <tr>
                                            <td>${o.curCode}</td>
                                            <td>${o.curName}</td>
                                            <td>${o.isapprove?'<i class="fa fa-check" aria-hidden="true"></i>':''}</td>
                                            <td>${o.isactive?'<i class="fa fa-check" aria-hidden="true"></i>':''}</td>
                                            <td>${o.decisionNo}</td>
                                            <td>${o.incharge}</td>
                                            <td><a style="font-size: 0.5em" href="./editcurriculum?curid=${o.curid}"><i class="btn btn-square bi bi-pencil-square"></i></a></td>
                                        </tr>
                                    </c:forEach>
                                </tbody>
                            </table>
                        ...
                        <nav aria-label="Page navigation example">
                            <ul class="pagination">
                                <c:if test="${tag > 1}">
                                    <li class="page-item">
                                        <a class="page-link" href="managecurriculum?page=${tag-1}" aria-label="Previous">
                                ...
                                <c:forEach begin="1" end="${endPage}" var="i">
                                    <li class="page-item ${tag == i ? "active" : ""}"><a class="page-link" href="managecurriculum?page=${i}">${i}</a></li>
                                    </c:forEach>
                                    <c:if test="${tag != endPage}"> 
                                    <li class="page-item">
                                        <a class="page-link" href="managecurriculum?page=${tag+1}" aria-label="Next">
