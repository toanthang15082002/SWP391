 ...
 * @author ADMIN
 */
@WebServlet(name = "SortAssessmentServletController", urlPatterns = {"/sortassessment"})
public class SortAssessmentServletController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SortAssessmentServletController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet SortAssessmentServletController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sort.equals("aid desc")) {
            List<Assessment> sortlist = aDao.sort(Integer.parseInt(syllabusID_raw), sort);
            int numPerPage = 2;
            ...
            request.setAttribute("listass", aDao.listByPage(sortlist, start, end));
            request.setAttribute("start", start);
            request.setAttribute("end", end);
            request.setAttribute("page", page);
            request.setAttribute("numpage", numberOfPage);
        } else if (sort.equals("aid")) {
            List<Assessment> sortlist = aDao.sort(Integer.parseInt(syllabusID_raw), sort);
            int numPerPage = 2;
            int size = sortlist.size();
            int page = 1;
            ...
            request.setAttribute("page", page);
            request.setAttribute("numpage", numberOfPage);
        } else if (sort.equals("categoti desc")) {
            List<Assessment> sortlist = aDao.sort(Integer.parseInt(syllabusID_raw), sort);
            int numPerPage = 2;
            int size = sortlist.size();
            int page = 1;
            ...
            request.setAttribute("page", page);
            request.setAttribute("numpage", numberOfPage);
        } else if (sort.equals("weight desc")) {
            List<Assessment> sortlist = aDao.sort(Integer.parseInt(syllabusID_raw), sort);
            int numPerPage = 2;
            int size = sortlist.size();
            int page = 1;
